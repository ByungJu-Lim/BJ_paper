...
- Git
- Python 3.11 이상 (검증 스크립트는 표준 라이브러리만 사용하므로 별도 패키지 설치가 필요 없습니다)
...
모든 도구는 Python 3.11 이상과 표준 라이브러리만 사용합니다.
...
- `rerun_manifest.py`는 `data/processed/<run-id>.manifest.json`에 기록된 명령을 실제로 재실행하고, 다시 만들어진 산출물을 커밋된 것과 바이트 단위로 비교합니다. `verify_story_brief.py --check-manifests`가 매니페스트의 형식과 파일 존재만 확인하는 것과 달리, 이쪽이 결과가 재현되는지를 답합니다. 원본은 재실행 전에 임시 위치로 옮겨두었다가 되돌리므로 이미 인용된 결과를 덮어쓰지 않습니다. 매니페스트에 `nondeterminism`을 적어두면 차이가 실패가 아니라 선언된 편차로 보고되지만, 차이 자체는 그대로 출력됩니다. 선언하지 않은 파일을 `data/processed/`에 쓰는 실행도 실패로 처리합니다 — 어떤 매니페스트도 가리키지 않는 결과 파일은 추적할 수 없기 때문입니다.
...
두 본문 검증기는 따옴표로 전달한 파일 패턴을 내부에서 확장하므로 PowerShell과 Bash에서 같은 명령을 사용합니다. 일치하는 파일이 없으면 실패합니다. CI는 Windows와 Ubuntu, Python 3.11과 3.14에서 실행합니다.
