...
| Finding | The black-box surrogate's extrapolation RMSE was ~2.9x its interpolation RMSE (supporting C1), but the preregistered transcription/unit/input-mapping alarm suspended C0 because the independently verified Ebert-Panchal correlation predicted zero net fouling across the entire protected training envelope, so the additive-residual hybrid's discrepancy term collapsed to fitting the same noisy signal as the black-box and was numerically worse on extrapolation in all 10 paired repetitions (p = 1.0 for the one-sided test favoring the hybrid) and identical to it on interpolation; C2 and C3 therefore cannot be adjudicated by this run. |
| Implication | Whether an additive-residual hybrid improves shell-and-tube fouling-resistance extrapolation over a matched-capacity black-box remains untested by this design, because the embedded correlation was uninformative (near-zero) over the chosen operating envelope rather than because the hybrid architecture failed; the next step is to redesign the envelope or select a correlation that stays informative throughout the training region so C0 can actually be adjudicated, and only then can C2/C3 be tested on their own terms. |
