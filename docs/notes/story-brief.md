...
| Gap | One physics-informed heat-exchanger fouling model we found enforces physics through loss-function penalties on a different target and exchanger geometry rather than an additive correlation-plus-residual structure (@jadhav2022physics), and purely data-driven shell-and-tube fouling models report no extrapolation-vs-interpolation comparison in their abstracts (@he2025machine, @hosseini2022novel; full texts not read), so we find no work evaluating an additive-residual physics-informed hybrid against a capacity-matched black-box under an explicit operating-condition split for shell-and-tube fouling resistance in a controlled simulation study; validating any such advantage against measured fouling data is left to future work. |
